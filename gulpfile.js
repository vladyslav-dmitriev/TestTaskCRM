...
var paths = {
	...
	dev: {
		...
		// Пути для обработки sass/css-файлов
		css: {
			src: './dev/styles/main.sass', // Файл который будет обработан задачей "css"
			dest: './dev/assets/css' // Директория в которую будут выведены рузультаты работы задачи "css"
		},
		// Пути для обработки js-файлов
		js: {
			src: './dev/scripts/**/*.js', // Все js-файлы в директории "scripts"
	...
	watch: {
		...
		sass: './dev/styles/**/*.sass', // Все sass-файлы в директории "sass/"
...
gulp.task('server', function() {
	...
	gulp.watch(paths.watch.sass, ['css']); // Cлежение за Sass-файлами
...
// Преобразование sass-файорв в css c оптимизацией и минификацией:
gulp.task('css', function() {
	return gulp
		.src(paths.dev.css.src) // Источник sass-файла(ов)
		.pipe(plumber()) // Обработка ошибок при работе плагина sass
		.pipe(sourcemaps.init()) // Инициализация sourcemaps
		.pipe(sass())
		.on('error', notify.onError()) // Преобразование sass-файлов в css, вывод уведомления в случае ошибки
	...
	var spriteData = gulp
		...
		.pipe(
			spritesmith({
				...
				cssName: '_sprite.sass', // Название итогового файла стилей
				cssFormat: 'sass', // Формат файла содержащего стили
